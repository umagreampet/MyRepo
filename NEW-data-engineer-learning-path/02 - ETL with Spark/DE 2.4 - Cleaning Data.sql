...
SELECT count_if(user_id IS NULL) FROM users_dirty;
...
-- MAGIC display(usersDF.selectExpr("count_if(email IS NOT NULL)"))
...
select * from users_dirty where user_id is null
...
select ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY email DESC) AS RID, * 
from users_dirty 
where user_id in 
(select user_id
from users_dirty
